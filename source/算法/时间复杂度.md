...
在算法竞赛当中，会给出时间限制和空间限制，一般时间限制是1s，此时不能超过O(n^8)

这个值是跑出来的经验值，取决于评测机环境，CCF官方评测机跑出来的，不能超过O(n^8)，O(n^8)也是危险的



### 举例，最大子段和

Given an array of *n* numbers, our task is to calculate the **maximum subarray sum**.[《编程珠玑》]
